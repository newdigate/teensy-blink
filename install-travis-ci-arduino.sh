...
export MAIN_PLATFORMS='declare -A main_platforms=( [teensy]="Teensyduino:samd:teensy36" [zero]="arduino:samd:arduino_zero_native" [m4]="adafruit:samd:adafruit_metro_m4" )'
...
DEPENDENCY_OUTPUT=$(arduino --pref "boardsmanager.additional.urls=https://adafruit.github.io/arduino-board-index/package_adafruit_index.json,https://github.com/newdigate/teensy-build/raw/master/package_teensyduino_index.json,http://arduino.esp8266.com/stable/package_esp8266com_index.json,https://dl.espressif.com/dl/package_esp32_index.json" --save-prefs 2>&1)
if [ $? -ne 0 ]; then echo -e """$RED""\xe2\x9c\x96"; else echo -e """$GREEN""\xe2\x9c\x93"; fi
...
echo -n "TEENSYDUINO: "
DEPENDENCY_OUTPUT=$(arduino --install-boards Teensyduino:samd 2>&1)
if [ $? -ne 0 ]; then echo -e "\xe2\x9c\x96 OR CACHED"; else echo -e """$GREEN""\xe2\x9c\x93"; fi
