...
export MAIN_PLATFORMS='declare -A main_platforms=( [teensy]="teensyduino:avr:teensy36:usb=serial,speed=180,opt=o2std,keys=en-us" )'
...
echo -e "\n########################################################################";
echo -e "${YELLOW} arm-none-eabi-g++ --version"
echo "########################################################################";
...
function build_platform()
{
  ...
  for example in "${examples[@]}"; do
    ...
    # "when declaring a local variable in a function, the local acts as a command in its own right"
    local build_stdout
    build_stdout=$(arduino --verify --verbose $example 2>&1)
